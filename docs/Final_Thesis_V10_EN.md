...
| Existing Chinese-calligraphy work (e.g. [2], [3], [29], [30]; UniCalli ICLR 2026; Calligrapher 2025) | Mature pipelines for logographic scripts; column-level synthesis; multilingual extension | No tone-mark evaluation metric; no Vietnamese diacritic accuracy reported |
| Existing Vietnamese text-to-image work (e.g. VietHann/T2I, Aug 2025) | Vietnamese prompt support; DiT-based foundation architecture | Multilingual CLIP/SigLIP tokenization fragments stacked diacritics; FID/CLIPScore-only evaluation; no code or fine-tuning recipe for calligraphy; no character-level accuracy target |
