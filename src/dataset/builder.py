...
def fit_font_for_block(
    ...
    *,
    ...
    for size in range(max_size, min_size - 1, -2):
        ...
        heights = []
        for line in lines:
            b = text_bbox(dummy, line, font)
            widths.append(b[2] - b[0])
            heights.append(b[3] - b[1])
        total_h = sum(heights) + max(0, len(lines) - 1) * size * line_gap_ratio
...
def draw_text_centered(
    ...
    *,
    ...
    return {
        ...
        "draw_xy": [round(x, 2), round(y, 2)],
        "target_box": [round(v, 2) for v in box],
        "ink_size": [tw, th],
    }

R = Path(__file__).resolve().parents[2]
LEX = R / "assets/vietnamesesyllable_7184.txt"
FONT = R / "assets/fonts/Thu_Phap_Thanh_Cong_Unicode.ttf"
TOK_DIR = "models/ideogram-ai/ideogram-4-fp8/tokenizer" # local tokenizer path or HuggingFace ID
ELIM = R / "assets/elim_state_soup567.json"
